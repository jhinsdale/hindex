...
from any section of the file by line number, seeking directly to the
target region rather than scanning from the beginning — making it
orders of magnitude faster than tools like `grep`, `sed -n`, or `awk`
on very large files.  In the special case where a leading portion of
each line is ordered, *e.g.*, for timestamped log files or sorted data
files, `hindex` can extract based on ranges of this beginning-of-line
content.
...
> **Note:** The C version supports **only single-letter options** (e.g. `-S`),
> not long-form options (e.g. `--start`).  The Python version supports both.
...
- [Usage](#usage)
  - [Options for mode of operation](#options-for-mode-of-operation)
  - [Search options](#search-options)
  - [Output options](#output-options)
  - [Index build options](#index-build-options)
  - [Index file name and location options](#index-file-name-and-location-options)
- [Examples](#examples)
...
```bash
# Index a large file, capturing 20-byte line prefixes for content search
hindex -P 20 -b -F /path/to/bigfile.log

# Extract lines 1000–2000 by line number
hindex -S 1000 -E 2000 -F /path/to/bigfile.log

# Extract lines whose content falls in a range (requires -P at index time)
hindex -G '2024-01-15' -L '2024-01-16' -F /path/to/bigfile.log
```

## Requirements

**Python version (`hindex.py`):**
- Python 3.x (no external dependencies; adjust the shebang if Python 3 is not at `/usr/bin/python3`)

**C version (`hindex`):**
- `gcc`
- `make`
- `libssl-dev` (provides `libcrypto`, used to compute SHA-1 index filenames)
...
`hindex -P 20 -b -F /usr/local/data/sample.txt`
...
> **Note:** Content-based search requires the index to have been built
> with `-P <bytes>`/`--snaplen <bytes>` (as shown in the build example above).
...
running `make`.  The `Makefile` is specific to Linux and `gcc`, and
requires the `libssl-dev` package (for `libcrypto`).  With adjustments
it should be able to be made to compile on other platforms.

Once built, run `make install` to copy the executables to the install
location.  The default install directory is `/usr/local/bin/`; this
can be overridden on the command line:

```bash
make install INSTALL_BIN_DIR=/your/preferred/bin
```

# Index file format

Each data file has a corresponding index file which, unless
